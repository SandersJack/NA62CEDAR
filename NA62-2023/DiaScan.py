...
#directory = '/eos/user/c/cparkins/public/DSCAN_202bar_2/'
...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        #file_name = "/eos/user/c/cparkins/public/PRESS_1mm/BrickA.34.root"
        ...
        '''
        bins_sel1 = h_NRecoHits.axis().edges()
        weight_sel1 = h_NRecoHits.counts()
        
        value1 = []
        for i in range(len(bins_sel1[1:])):
            for t in range(int(weight_sel1[i])):
                value1.append(bins_sel1[i])

        mu1, std = norm.fit(value1)
        '''
        mu1 = h_NRecoHits.member("fTsumwx")/h_NRecoHits.member("fEntries")
        mu_Hits.append(mu1)
        mu_Hits_Arg.append(mu1/(Arg_count))#/230640000))
        
        '''
        bins_cand2 = h_NHitsInCandidate.axis().edges()
        weight_cand2 = h_NHitsInCandidate.counts()
        
        
        value2 = []
        for i in range(len(bins_cand2[1:])):
            for t in range(int(weight_cand2[i])):
                value2.append(bins_cand2[i])

        mu2, std = norm.fit(value2)
        '''
        mu2 = h_NHitsInCandidate.member("fTsumwx")/h_NHitsInCandidate.member("fEntries")
        mu_CandHits.append(mu2)
        mu_CandHits_Arg.append(mu2/(Arg_count))#230640000))
        '''
        bins_cand3 = h_NRecoHitsInSelectedCandidate.axis().edges()
        weight_cand3 = h_NRecoHitsInSelectedCandidate.counts()
        
        value3 = []
        for i in range(len(bins_cand3[1:])):
            for t in range(int(weight_cand3[i])):
                value3.append(bins_cand3[i])

        mu3, std = norm.fit(value3)
        '''
        try:
        ...
        mu_SelCandHits_Arg.append(mu3/(Arg_count))#230640000))
...
axs6.scatter(burst,mu_Hits_Arg,marker="x",label="Reco")
axs6.scatter(burst,mu_CandHits_Arg,marker="x",label="Cand")
axs6.scatter(burst,mu_SelCandHits_Arg,marker="x",label="Sel Cand")

axs6.set_ylabel("Number of Hits Normalise to Argonion")
axs6.set_xlabel("Burst")
axs6.xaxis.set_major_locator(plt.MaxNLocator(20))
axs6.tick_params(axis='x', labelrotation = 90)
axs6.legend()
'''
